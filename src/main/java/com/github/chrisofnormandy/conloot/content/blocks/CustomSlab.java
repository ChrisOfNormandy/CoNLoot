...
public class CustomSlab {
    ...
    public static void registerBlockFromConfig(String name, Config config, Groups blockGroup) {
        ...
        switch (type) {
            case "stone": {
                ModRegister.registerBlock(name, Subsets.create_slab(ModBlock.Stone.create(harvestLevel, strength)), blockGroup);
                break;
            }
            case "wood": {
                ModRegister.registerBlock(name, Subsets.create_slab(ModBlock.Wood.create(harvestLevel, strength)), blockGroup);
                break;
            }
            case "bricks": {
                ModRegister.registerBlock(name, Subsets.create_slab(ModBlock.Bricks.create(harvestLevel, strength)), blockGroup);
                break;
            }
            default: {
                ModRegister.registerBlock(name, Subsets.create_slab(ModBlock.Stone.create(harvestLevel, strength)), blockGroup);
